...
import { canBeat, canChop, checkWhiteWin, parseCombo } from '../src/rules/index.js';
function c(code, idx = 0) {
            ...
            : r === 'K'
                ...
                : r === 'A'
                    ...
                    : r === '2'
                        ? 15
                        : Number(r);
    return { id: `${code}:${idx}`, rank, suit: s };
}
describe('parseCombo', () => {
    test('parses single', () => {
...
describe('canBeat', () => {
    test('single compares suit when ranks equal', () => {
        const last = parseCombo([c('3S')]);
        const play = parseCombo([c('3H')]);
        expect(canBeat(play, last)).toBe(true);
    });
    test('pair compares rank', () => {
        const last = parseCombo([c('5H'), c('5S')]);
        const play = parseCombo([c('6H'), c('6S')]);
        expect(canBeat(play, last)).toBe(true);
    });
    test('straight compares highest rank only (S1), length must match', () => {
        const last = parseCombo([c('3H'), c('4S'), c('5D')]);
        const play = parseCombo([c('6H'), c('7D'), c('8S')]);
        expect(canBeat(play, last)).toBe(true);
        const playLen4 = parseCombo([c('6H'), c('7D'), c('8S'), c('9C')]);
        expect(canBeat(playLen4, last)).toBe(false);
    });
    test('cannot beat with different combo type', () => {
        const last = parseCombo([c('3H')]);
        const play = parseCombo([c('4H'), c('4S')]);
        expect(canBeat(play, last)).toBe(false);
    });
});
describe('canChop (P1)', () => {
    test('3 consecutive pairs chops single 2', () => {
        const last = parseCombo([c('2S')]);
        const play = parseCombo([c('3H'), c('3D'), c('4C'), c('4S'), c('5H'), c('5D')]);
        expect(canChop(play, last)).toBe(true);
    });
    test('four of a kind chops single 2', () => {
        const last = parseCombo([c('2S')]);
        const play = parseCombo([c('9H'), c('9D'), c('9C'), c('9S')]);
        expect(canChop(play, last)).toBe(true);
    });
    test('4 consecutive pairs chops pair of 2', () => {
        const last = parseCombo([c('2H'), c('2S')]);
    ...
    test('four of a kind chops consecutive pairs', () => {
        const last = parseCombo([c('3H'), c('3D'), c('4C'), c('4S'), c('5H'), c('5D')]);
        const play = parseCombo([c('9H'), c('9D'), c('9C'), c('9S')]);
        expect(canChop(play, last)).toBe(true);
    });
